...
-- And a lot of these tools aren't really groundbreaking anyway.

local NumberTools = {}
local INFINITY = utf8.char(8734) -- Infinity symbol

-- This one has "G" for "Giga". Lower line has "B" for "Billion". Same thing. Most people see 100B as 100 billion, and 100G as ... really good cell service?
...
local TRUNCATE_LABELS = {"k", "M", "B", "T", "P", "E", "Z", "Y"}
...
-- Round
-- Accepts a number and an optional places (where 0 = 1s place (whole number), 1 = 10s place, 2 = 100s place, ...)
-- Returns this number rounded to the given place.
function NumberTools.Round(number, places)
...
function NumberTools.TruncateNumber(number, placeCount)
	...
	if resultingValue < 1 then
		...
		resultingValue = NumberTools.Round(resultingValue * 1000, placeCount)
	...
	repeat
		local letterIndex = math.clamp(count, 1, 8)
...
function NumberTools.Clamp01(value)
	return math.clamp(value, 0, 1)
end

-- Alias. Linear Interpolation from start -> goal by alpha%
function NumberTools.Lerp(start, goal, alpha)
...
function NumberTools.Map(x, inMin, inMax, outMin, outMax)
	return (x - inMin) * (outMax - outMin) / (inMax - inMin) + outMin;
end

-- Alias to mapping a given value to a new value that is between 0 and 1
function NumberTools.Map01(x, min, max)
	return (x - min) / max
end

-- Given a number, this will add commas every three digits, so 2718391 will return as "2,718,391"
function NumberTools.WithCommas(value, useAltCommas)
